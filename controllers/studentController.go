...
var SentQRCode = func(w http.ResponseWriter, r *http.Request) {
	...
	err = models.GetDB().Table("schedules").Where("schedules.group = ? AND time > ? AND time < ? AND day = ?", ss.Group, curTime.Add(-40*time.Minute+3*time.Hour).Format("15:04:05"), curTime.Add(10*time.Minute+3*time.Hour).Format("15:04:05"), strings.ToLower(time.Now().Weekday().String())).First(&s).Error
